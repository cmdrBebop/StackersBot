...
class SubscribeWorker(Worker):
    ...
    async def add_subscribes(self, telegram_id: int, hackaton_sub: bool = True, lecture_sub: bool = True,
    ...
    async def get_subscribes(self, telegram_id: int) -> asyncpg.Record:
        sql = f'''
        SELECT hacaton_subscribe, lecture_subscribe, meet_up_subscribe, vacancy_subscribe FROM {self.table_name} WHERE telegram_id={telegram_id}
        '''
    ...
    async def update_subscribe(self, telegram_id: int, subscribe: str, sub_status: bool) -> None:
        sql = f'''
        UPDATE {self.table_name} SET {subscribe}={sub_status} WHERE telegram_id={telegram_id}
        '''
